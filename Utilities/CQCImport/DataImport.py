...
class DataImport:
    def __init__(self, id_found, dbImport):
        self.args = {}
        self.db = dbImport
    ...
    def insert_funding(self, args):
        ...
        try:
            db.execute(self.db, stmt, args, "prepared", False)
        except mysql.connector.Error as err:
            print(args)
            print(err)

    def insert_link_taxonomy(self, args):
    ...
    def insert_service_taxonomy(self):
        ...
        for i in range(len(self.args["service_taxonomy"]) // 50):
            ...
            stmt = "INSERT INTO service_taxonomy (id, service_id, taxonomy_id) VALUES " + ",".join(
                "(%s, %s, %s)" for _ in flat[50 * i:50 * (i + 1)])
            stmt += "ON DUPLICATE KEY UPDATE " \
    ...
    def delete_service(self, id: str):
        stmt = """DELETE FROM service WHERE id = %s"""
        c = self.db.cursor(prepared=True)
