...
class AddUUID:
    def __init__(self, db, origin):
        ...
        self.services = []
        self.new_uuid = []
        self.uuid = {"organization": self.get_uuid_lookup("organization", self.origin),
                     "service": self.get_uuid_lookup("service", self.origin),
                     "location": self.get_uuid_lookup("location", self.origin)}
        pass

    def insert_new_uuid(self):
        print("Inserting UUIDs")
    ...
    def get_uuid(self, external_id, table, origin):
        if self.uuid[table] is not None:
            if (str(external_id), origin) in self.uuid[table]:
                return self.uuid[table][str(external_id), origin][0]
