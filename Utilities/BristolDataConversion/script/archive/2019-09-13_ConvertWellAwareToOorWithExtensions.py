...
def setupService(row):
    ...
    if "Address" in row:
        if (row["Address"]):
            ...
            if not(Postcode):
                if row["Postcodeareawheretheorganisationworks"]:
...
for row_in_Organisations in csv_reader_Organisations:
    ...
    if idOrganization:
        ...
        if "Title" in row_in_Organisations:
            if row_in_Organisations["Title"].strip():
                row_organization.update({'name': row_in_Organisations["Title"].strip()})

        if "ImageURL" in row_in_Organisations:
            if row_in_Organisations["ImageURL"].strip():
                row_organization.update({'logo': row_in_Organisations["ImageURL"].strip()})

        if "Websiteurl" in row_in_Organisations:
            if row_in_Organisations["Websiteurl"].strip():
                row_organization.update({'url': row_in_Organisations["Websiteurl"].strip()})
